...
function romanToNumberconvert(romanLetter, lastIndex, totalValue, previousValue) { // II, III, IV,VI, XIV 
  if (lastIndex > -1) {
    let currentValue = romanToNumberMap(romanLetter[lastIndex]);  
    ...
    return romanToNumberconvert(romanLetter, lastIndex - 1, totalValue, previousValue)
  }
  return totalValue;
}

function romanToNumber(romanLetter) {
  const lengthOfRomanLetter = romanLetter.length - 1;

  return romanToNumberconvert(romanLetter, lengthOfRomanLetter, 0, 0);

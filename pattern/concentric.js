...
function sideDistance(p1, center) {
  const deltaH = p1[0] - center[0];
  const deltaW = p1[1] - center[1];
  return Math.abs(deltaH + deltaW) + Math.abs(deltaH - deltaW);
}

function drawSquare(p1, centerPoint, sideLength) {
  let d = sideDistance(p1, centerPoint);
  return chooseChar(d, sideLength);
}
  ...
  for (let h = 0; h < height; h++) {
    ...
    for (let w = 0; w < width; w++) {
      ...
      if (patternType === "square") {
        ...
        const cw = 5;
        const sideLength = 10;
        let square = drawSquare([h, w], [ch, cw], sideLength);
        line += square;
      }
    }
    lines.push(line);
